...
const WS_URL = "wss://mtutrade.in/ws/vajra"
const INSTRUMENTS = {
...
const PositionRow = memo(({ p, ceLtp, peLtp, instr, onClose, T }) => {
  ...
  return (
    <div style={{
      display: "grid", gridTemplateColumns: "2fr .7fr .7fr .7fr .7fr 1fr",
      padding: "9px 4px", borderBottom: `1px solid ${T.line}`,
      alignItems: "center", background: T.surface, minHeight: 44
    }}>
      <div>
      ...
      <div style={{ display: "flex", gap: 4 }}>
        <button onPointerDown={() => onClose(p.id, curLtp || p.target_price, "TARGET")}
          style={{ minHeight: 28, padding: "0 6px", borderRadius: 5, border: "none",
            background: T.buy, color: "#fff", fontSize: 10, fontWeight: 600,
            cursor: "pointer", WebkitTapHighlightColor: "transparent", touchAction: "manipulation" }}>Exit</button>
        <button onPointerDown={() => onClose(p.id, p.sl, "SL")}
          style={{ minHeight: 28, padding: "0 6px", borderRadius: 5, border: "none",
            background: T.sell, color: "#fff", fontSize: 10, fontWeight: 600,
            cursor: "pointer", WebkitTapHighlightColor: "transparent", touchAction: "manipulation" }}>SL</button>
      </div>
    </div>
  )
})
export default function App({ user, onLogout }) {
  ...
  useEffect(() => {
    ...
    if (tk) {
      fetch("https://mtutrade.in/api/auth/broker/my-brokers", {
        headers: { "Authorization": "Bearer " + tk }, credentials: "include"
      }).then(r => r.json()).then(d => {
        if (d.brokers && Object.keys(d.brokers).length > 0) setBrokerOk(true)
      }).catch(() => {})
    }
    ...
    const startPolling = () => {
      const pollMarket = async () => { const r = await api("/vajra/market"); if (r) setMarket(r) }
      const pollLtp = async () => {
        if (!ceKey || !peKey) return
        const r = await api(`/sutra/ltp?ce_key=${encodeURIComponent(ceKey)}&pe_key=${encodeURIComponent(peKey)}`)
        if (!r) return
        Object.entries(r).forEach(([k, v]) => {
          if (k.includes("CE")) { setCeLtp(v); marketTickRef.current.ce = v }
          if (k.includes("PE")) { setPeLtp(v); marketTickRef.current.pe = v }
        })
      }
      ...
      pollLtp(); const t2 = setInterval(pollLtp, 2000)
      pollState(); const t3 = setInterval(pollState, 10000)
      pollingTimers.current = [t1, t2, t3]
    }
    ...
    const connectWs = () => {
      try {
        ...
        wsRef.current = ws
        ws.onopen = () => {
          ws.send(JSON.stringify({ action: "subscribe", channels: ["market", "ltp", "state"] }))
          ...
          try {
            ...
            if (msg.type === "ltp") {
              marketTickRef.current = { 
                ce: msg.ce ?? marketTickRef.current.ce, 
                pe: msg.pe ?? marketTickRef.current.pe,
                spot: msg.spot ?? marketTickRef.current.spot
              }
              updateLtpDom(msg.ce, msg.pe, msg.spot)
              if (!renderFrame.current) {
                renderFrame.current = setTimeout(() => {
                  const { ce, pe } = marketTickRef.current
                  if (ce != null) setCeLtp(ce)
                  if (pe != null) setPeLtp(pe)
            ...
            if (msg.type === "state") {
              setPragnya(msg.data)
              if (msg.data?.trades) setPositions(msg.data.trades.filter(t => t.status === "OPEN"))
            }
          } catch (err) {}
        }
  ...
  const onCeChange = useCallback((val) => {
    ...
    if (r) { setCeLtp(r.ce.ltp); setCeKey(r.ce.key) }
  }, [strikes])

  const onPeChange = useCallback((val) => {
    setPeStrike(val)
    const r = strikes.find(s => String(s.strike) === val)
    if (r) { setPeLtp(r.pe.ltp); setPeKey(r.pe.key) }
  }, [strikes])
  const execute = useCallback((type) => {
    if (execLock.current) {
      const existing = orderQueue.current.find(q => q.type === type && q.status === 'pending')
      if (existing) {
        existing.qty += qty
        showToast(`${type} ×${existing.qty} queued`, true)
        return
      }
      orderQueue.current.push({ type, qty, status: 'pending' })
      showToast(`${type} queued`, true)
      return
    }
    ...
    if (!strike) {
      showToast("Select a strike first", false)
      execLock.current = false
      setExecBusy(false)
      return
    }
    
    showToast(`${type} sending...`, true)
    
    api("/vajra/trade/open", {
      method: "POST",
      body: JSON.stringify({
      ...
      if (r?.status === "ok") {
        ...
        setPositions(prev => [...prev, {
          id: r.trade_id || Date.now(), instrument: `${symbol}${strike}${optType}`,
          direction: action, entry: ltp,
          sl: action === "SELL" ? (ltp || 0) + slPts : (ltp || 0) - slPts,
          target: action === "SELL" ? (ltp || 0) - tgtPts : (ltp || 0) + tgtPts,
          status: "OPEN", extra_json: JSON.stringify({ lots: qty })
        }])
        return api("/vajra/state")
      } else {
  ...
  }, [ceLtp, peLtp, ceStrike, peStrike, symbol, qty, slPts, tgtPts, showToast])

  const flushQueue = useCallback(() => {
    if (flushTimer.current) return
    flushTimer.current = setTimeout(() => {
      while (orderQueue.current.length > 0 && !execLock.current) {
        const next = orderQueue.current.shift()
  ...
  const closePos = useCallback(async (id, ep, reason) => {
    const r = await api("/vajra/trade/close", {
      method: "POST",
      body: JSON.stringify({ trade_id: id, exit_price: ep || 0, exit_reason: reason })
    })
    if (r?.status === "ok") {
  ...
  const closeAll = useCallback(async () => {
    ...
    const closedSnapshot = [...positions]
    setPositions([])
    showToast(`Closing ${closedSnapshot.length} positions...`, true)
    
    const results = await Promise.allSettled(
      closedSnapshot.map(p => {
        const cl = p.instrument.includes("CE") ? ceLtp : peLtp
        return api("/vajra/trade/close", {
          method: "POST",
          body: JSON.stringify({
            trade_id: p.id, exit_price: cl || p.sl || 0, exit_reason: "MANUAL"
          })
        })
      })
    )
    ...
    const failCount = closedSnapshot.length - successCount
    
    if (failCount > 0) {
      showToast(`${failCount} close failed — restoring`, false)
      const failed = closedSnapshot.filter((_, i) =>
        !(results[i].status === "fulfilled" && results[i].value?.status === "ok")
      )
      setPositions(prev => [...prev, ...failed])
    } else {
      showToast(`✓ All ${successCount} positions closed`, true)
    }
    
    const pr = await api("/vajra/state")
    if (pr?.trades) setPositions(pr.trades.filter(t => t.status === "OPEN"))
    if (pr) setPragnya(pr)
  }, [positions, ceLtp, peLtp, showToast])
  ...
  const lbl = t => (
    <div style={{
      fontFamily: mono, fontSize: 8, fontWeight: 600, color: T.subtle,
      letterSpacing: "1.5px", textTransform: "uppercase", marginBottom: 3
    }}>{t}</div>
  )

  const ExecBtn = ({ text, sub, onClick, color }) => (
    <button onPointerDown={onClick} disabled={execBusy}
      style={{
        display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
        width: "100%", minHeight: isMobile ? 56 : 52, gap: 1,
        borderRadius: 8, border: "none", background: execBusy ? T.line : color,
        color: "#fff", fontFamily: inter, cursor: execBusy ? "not-allowed" : "pointer",
        WebkitTapHighlightColor: "transparent", touchAction: "manipulation",
        opacity: execBusy ? 0.5 : 1,
        boxShadow: execBusy ? "none" : (dark ? `0 2px 8px ${color}66` : `0 2px 4px ${color}44`),
        transition: "opacity 0.1s, transform 0.05s",
      }}
      onPointerDownCapture={e => { if (!execBusy) e.currentTarget.style.transform = "scale(0.97)" }}
    ...
    <div style={{ minHeight: "100vh", background: T.canvas, fontFamily: inter }}>
      <div style={{
        ...
        <button onPointerDown={() => setAppScreen("terminal")}
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "16px" }}>
        <div style={{ background: T.surface, borderRadius: 12, padding: "16px", border: `1px solid ${T.line}`, marginBottom: 12 }}>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {(user?.products || ["VAJRA"]).map(p => (
              <span key={p} style={{
                fontFamily: mono, fontSize: 9, fontWeight: 700, color: T.brand,
                background: dark ? "#2A1A0A" : "#FFF3E0", border: `1px solid ${T.brand}`,
        ...
        <div style={{ background: T.surface, borderRadius: 12, padding: "16px", border: `1px solid ${T.line}`, marginBottom: 12 }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <div onPointerDown={() => setDark(d => !d)}
              style={{
                width: 48, height: 26, borderRadius: 100, background: dark ? T.brand : T.line,
        ...
        <button onPointerDown={handleLogout}
          style={{
            ...
            border: `1.5px solid ${T.sell}`, background: "transparent", color: T.sell,
  ...
  if (gitaMsg) return (
    <div style={{
      ...
      background: dark ? "rgba(10,10,10,0.97)" : "rgba(250,249,247,0.97)",
      zIndex: 99999, display: "flex", flexDirection: "column",
      alignItems: "center", justifyContent: "center", padding: 28, fontFamily: inter
    }}>
      ...
      <button onPointerDown={() => setGitaMsg(null)}
        style={{
          minHeight: 44, padding: "10px 28px", borderRadius: 8,
  ...
  const DrawerContent = () => (
    <div style={{
      width: 300, background: T.surface, height: "100%", display: "flex",
      flexDirection: "column", boxShadow: "-4px 0 24px rgba(0,0,0,0.15)", overflowY: "auto"
    }}>
      <div style={{
        ...
        <button onPointerDown={() => setDrawer(false)}
          style={{ background: "none", border: "none", fontSize: 18, cursor: "pointer", color: T.subtle, WebkitTapHighlightColor: "transparent" }}>✕</button>
      </div>
      ...
      <div style={{ display: "flex", borderBottom: `1px solid ${T.line}`, flexShrink: 0 }}>
        {[["broker", "🔗 Broker"], ["appearance", "🌙 Theme"], ["pragnya", "🧠 Pragnya"], ["gita", "🕉️ Gita"]].map(([k, l]) => (
          <button key={k} onPointerDown={() => setDrawerTab(k)}
            style={{
              flex: 1, padding: "8px 4px", border: "none",
              borderBottom: drawerTab === k ? `2px solid ${T.brand}` : "2px solid transparent",
              background: "none", color: drawerTab === k ? T.brand : T.subtle,
              fontFamily: inter, fontWeight: 600, fontSize: 10, cursor: "pointer",
              WebkitTapHighlightColor: "transparent", touchAction: "manipulation"
            }}>{l}</button>
        ))}
      </div>
      ...
      <div style={{ padding: "14px 16px", borderTop: `1px solid ${T.line}`, flexShrink: 0 }}>
        <button onPointerDown={handleLogout}
          style={{
            width: "100%", minHeight: 42, borderRadius: 8,
            border: `1.5px solid ${T.sell}`, background: "transparent", color: T.sell,
    ...
    <div style={{ minHeight: "100vh", background: T.canvas, fontFamily: inter, transition: "background .3s" }}>
      ...
      {toast && <div style={{
        ...
        background: toast.ok ? T.buy : T.sell, padding: "11px 16px",
        fontSize: 13, fontWeight: 600, color: "#fff", textAlign: "center", fontFamily: inter
      }}>{toast.msg}</div>}
      
      {loading && <div style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 9998, height: 2, background: T.brand }} />}
      
      {!brokerOk && appScreen === "terminal" && (
        <div onPointerDown={() => setAppScreen("settings")}
          style={{
            background: "#7B5800", color: "#fff", padding: "9px 16px",
            display: "flex", alignItems: "center", justifyContent: "space-between",
            cursor: "pointer", WebkitTapHighlightColor: "transparent"
          }}>
          <span style={{ fontFamily: "Inter,sans-serif", fontSize: 12, fontWeight: 600 }}>⚠️ No broker connected — market data unavailable</span>
          <span style={{ fontFamily: "Inter,sans-serif", fontSize: 12, fontWeight: 700 }}>Connect →</span>
        </div>
      )}
      ...
      <div style={{
        background: T.surface, borderBottom: `1px solid ${T.line}`,
        padding: "0 12px", height: 44, display: "flex", alignItems: "center", gap: 8,
        position: "sticky", top: 0, zIndex: 100, transition: "background .3s"
      }}>
        ...
        <div style={{
          display: "flex", gap: 12, alignItems: "center", overflowX: "auto", flex: 1,
          ...
          {[
            ...
            <div key={n} style={{ display: "flex", alignItems: "baseline", gap: 4, flexShrink: 0 }}>
              ...
              {chg !== 0 && <span style={{ fontFamily: mono, fontSize: 9, color: chg >= 0 ? T.up : T.down }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexShrink: 0 }}>
          <div style={{ textAlign: "right" }}>
            ...
            <div style={{ fontFamily: mono, fontSize: 12, fontWeight: 700, color: dayPnl >= 0 ? T.up : T.down }}>
          ...
          <button onPointerDown={() => setAppScreen("settings")}
            style={{
              width: 32, height: 32, borderRadius: 8, border: `1px solid ${T.line}`,
              background: T.raised, color: T.subtle, fontSize: 16, cursor: "pointer",
              WebkitTapHighlightColor: "transparent", touchAction: "manipulation",
      ...
      <div style={{
        ...
        padding: "8px 12px", transition: "background .3s",
        ...
        {isMobile ? (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px 10px" }}>
            ...
            <div>
              ...
              <div style={{ display: "flex", alignItems: "center", background: T.surface, border: `1px solid ${T.line}`, borderRadius: 6, height: 32 }}>
                <button onPointerDown={() => setQty(q => Math.max(1, q - 1))}
                  style={{ flex: 1, height: 32, background: "none", border: "none", cursor: "pointer", fontWeight: 700, fontSize: 16, color: T.ink, WebkitTapHighlightColor: "transparent", touchAction: "manipulation" }}>−</button>
                <span style={{ fontFamily: mono, minWidth: 24, textAlign: "center", fontWeight: 700, fontSize: 13, color: T.ink }}>{qty}</span>
                <button onPointerDown={() => setQty(q => q + 1)}
                  style={{ flex: 1, height: 32, background: "none", border: "none", cursor: "pointer", fontWeight: 700, fontSize: 16, color: T.ink, WebkitTapHighlightColor: "transparent", touchAction: "manipulation" }}>+</button>
              </div>
            </div>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
            ...
            <div>
              ...
              <div style={{ display: "flex", alignItems: "center", background: T.surface, border: `1px solid ${T.line}`, borderRadius: 6, height: 32 }}>
                <button onPointerDown={() => setQty(q => Math.max(1, q - 1))}
                  style={{ width: 28, height: 32, background: "none", border: "none", cursor: "pointer", fontWeight: 700, fontSize: 16, color: T.ink, WebkitTapHighlightColor: "transparent", touchAction: "manipulation" }}>−</button>
                <span style={{ fontFamily: mono, minWidth: 24, textAlign: "center", fontWeight: 700, fontSize: 13, color: T.ink }}>{qty}</span>
                <button onPointerDown={() => setQty(q => q + 1)}
                  style={{ width: 28, height: 32, background: "none", border: "none", cursor: "pointer", fontWeight: 700, fontSize: 16, color: T.ink, WebkitTapHighlightColor: "transparent", touchAction: "manipulation" }}>+</button>
              </div>
      ...
      <div style={{
        position: "sticky", top: isMobile ? 140 : 80, zIndex: 80,
        ...
        {isMobile ? (
          <div>
            <div style={{ padding: "10px 12px", borderBottom: `1px solid ${T.line}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                ...
                <div ref={spotRef} style={{ fontFamily: mono, fontSize: 28, fontWeight: 700, color: T.ink, lineHeight: 1, letterSpacing: "-0.5px" }}>
                  {indexLtp?.toLocaleString("en-IN", { maximumFractionDigits: 2 }) || "—"}
                </div>
                <div style={{ fontFamily: mono, fontSize: 11, fontWeight: 600, color: indexChg >= 0 ? T.up : T.down, marginTop: 2 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", borderBottom: `1px solid ${T.line}` }}>
              <div style={{ padding: "10px", borderRight: `1px solid ${T.line}` }}>
                ...
                <div ref={ceLtpRef} style={{ fontFamily: mono, fontSize: 30, fontWeight: 700, color: T.ink, lineHeight: 1, letterSpacing: "-0.5px", marginBottom: 2 }}>
                  {ceLtp != null ? ceLtp : "—"}
                </div>
              ...
              <div style={{ padding: "10px" }}>
                ...
                <div ref={peLtpRef} style={{ fontFamily: mono, fontSize: 30, fontWeight: 700, color: T.ink, lineHeight: 1, letterSpacing: "-0.5px", marginBottom: 2, textAlign: "right" }}>
                  {peLtp != null ? peLtp : "—"}
                </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, padding: "8px 10px" }}>
              <button onPointerDown={closeAll}
                style={{
                  minHeight: 36, borderRadius: 8, border: `1.5px solid ${T.sell}`,
                  background: "transparent", color: T.sell, fontFamily: inter,
                  fontWeight: 600, fontSize: 12, cursor: "pointer",
                  WebkitTapHighlightColor: "transparent", touchAction: "manipulation"
                }}>Close All</button>
              <button onPointerDown={() => showToast("Orders cancelled")}
                style={{
                  minHeight: 36, borderRadius: 8, border: `1.5px solid ${T.line}`,
                  background: "transparent", color: T.body, fontFamily: inter,
                  fontWeight: 600, fontSize: 12, cursor: "pointer",
                  WebkitTapHighlightColor: "transparent", touchAction: "manipulation"
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", background: T.surface, borderBottom: `1px solid ${T.line}`, transition: "background .3s" }}>
            <div style={{ padding: "14px", borderRight: `1px solid ${T.line}` }}>
              ...
              <div ref={ceLtpRef} style={{ fontFamily: mono, fontSize: 42, fontWeight: 700, color: T.ink, lineHeight: 1, letterSpacing: "-1px", marginBottom: 3 }}>
                {ceLtp != null ? ceLtp : "—"}
              </div>
            ...
            <div style={{ padding: "14px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "space-between", textAlign: "center", borderRight: `1px solid ${T.line}` }}>
              <div>
                ...
                <div ref={spotRef} style={{ fontFamily: mono, fontSize: 40, fontWeight: 700, color: T.ink, lineHeight: 1, letterSpacing: "-1px" }}>
                  {indexLtp?.toLocaleString("en-IN", { maximumFractionDigits: 2 }) || "—"}
                </div>
                <div style={{ fontFamily: mono, fontSize: 13, fontWeight: 600, color: indexChg >= 0 ? T.up : T.down, marginTop: 5 }}>
                  {indexChg >= 0 ? "+" : ""}{indexChg.toFixed(2)} ({indexPct >= 0 ? "+" : ""}{indexPct.toFixed(2)}%)
                </div>
              </div>
            ...
            <div style={{ padding: "14px" }}>
              ...
              <div ref={peLtpRef} style={{ fontFamily: mono, fontSize: 42, fontWeight: 700, color: T.ink, lineHeight: 1, letterSpacing: "-1px", marginBottom: 3, textAlign: "right" }}>
                {peLtp != null ? peLtp : "—"}
              </div>
      ...
      <div style={{ padding: "0 12px 40px" }}>
        <div style={{ display: "flex", alignItems: "center", borderBottom: `1px solid ${T.line}` }}>
          {[["positions", "Positions"], ["orders", "Orders"], ["journal", "Trade Book"]].map(([k, l]) => (
            <button key={k} onPointerDown={() => setTab(k)}
              style={{
                minHeight: 40, padding: "0 12px", border: "none",
                ...
                marginBottom: -1, WebkitTapHighlightColor: "transparent", touchAction: "manipulation"
              }}>{l}</button>
          ))}
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 6 }}>
            <span style={{ fontFamily: mono, fontSize: 9, color: T.subtle }}>MTM</span>
            <span style={{ fontFamily: mono, fontSize: 13, fontWeight: 700, color: dayPnl >= 0 ? T.up : T.down }}>
              {dayPnl >= 0 ? "+" : ""}₹{Math.abs(dayPnl).toLocaleString()}
            </span>
          </div>
        </div>
        ...
        <div style={{
          ...
          {tab === "positions" && (
            <div>
              <div style={{
                display: "grid", gridTemplateColumns: "2fr .7fr .7fr .7fr .7fr 1fr",
                padding: "7px 4px", background: T.raised, borderBottom: `1px solid ${T.line}`,
                position: "sticky", top: 0, zIndex: 10
              }}>
              ...
              {positions.length === 0
                ...
                : positions.map(p => (
                    <PositionRow key={p.id} p={p} ceLtp={ceLtp} peLtp={peLtp} instr={instr} onClose={closePos} T={T} />
                  ))
              }
            </div>
          )}
          ...
          {tab === "journal" && (
            <div>
              <div style={{
                display: "grid", gridTemplateColumns: "2fr .6fr .7fr .7fr .8fr .6fr",
                padding: "7px 4px", background: T.raised, borderBottom: `1px solid ${T.line}`,
                position: "sticky", top: 0, zIndex: 10
              }}>
              ...
              {(pragnya?.trades || []).filter(t => t.status === "CLOSED").length === 0
                ...
                : (pragnya?.trades || []).filter(t => t.status === "CLOSED").map(t => (
                    <div key={t.id} style={{
                      display: "grid", gridTemplateColumns: "2fr .6fr .7fr .7fr .8fr .6fr",
                      padding: "9px 4px", borderBottom: `1px solid ${T.line}`,
                      alignItems: "center", background: t.pnl >= 0 ? (dark ? "#0A1A0A" : "#F6FBF6") : (dark ? "#1A0A0A" : "#FDF6F6")
                    }}>
                      ...
                      <div style={{ fontFamily: mono, fontSize: 11, fontWeight: 700, color: t.pnl >= 0 ? T.up : T.down }}>
                        {t.pnl >= 0 ? "+" : ""}₹{Math.round(t.pnl || 0)}
                      </div>
