...
function Root() {
  ...
  useEffect(()=>{
    ...
    fetch(`${API}/auth/verify`, {
      ...
      // Network error — let user through if token exists (offline tolerance)
  ...
  if(checking) return (
    <div style={{minHeight:"100vh",background:"#FAF9F7",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:13,color:"#7A7670"}}>Loading VAJRA…</div>
    </div>
  )

  if(!user) return <Login onSuccess={(data)=>setUser(data)}/>

  return <App user={user} onLogout={()=>{ localStorage.removeItem("mtu_token"); localStorage.removeItem("mtu_user"); setUser(null) }}/>
}
