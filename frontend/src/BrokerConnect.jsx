  ...
  async function connect() {
    ...
    try {
      ...
      if(d.status==="ok") {
        ...
        window.location.href = `${API}/auth/broker/connect/${broker}?token=${jwt}`
      } else {
        ...
        {Object.entries(BROKERS).map(([key,info])=>{
          ...
          return (
            <button key={key} onPointerDown={()=>{ setBroker(key); setFields({}); setError("") }}
              style={{padding:"12px 8px",borderRadius:8,
                border:`1.5px solid ${isConnected?T.buy:T.line}`,
                background:isConnected?(T.canvas=="#0A0A0A"?"#0A1A0A":"#F0FFF4"):T.raised,
              ...
              {isConnected
                ?<div style={{fontFamily:"monospace",fontSize:9,color:T.buy,fontWeight:700}}>✓ Connected</div>
                :<div style={{fontFamily:"monospace",fontSize:9,color:T.subtle}}>Tap to connect</div>
              }
            </button>
          )
        })}
