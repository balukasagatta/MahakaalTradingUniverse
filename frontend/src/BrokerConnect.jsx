...
const BROKERS = {
  ...
  fyers:   { name:"Fyers",   logo:"🔵", dashboard:"https://myapi.fyers.in/", redirect:"https://mtutrade.in/api/auth/broker/fyers/callback", fields:[{key:"user_id",label:"Fyers ID",hint:"Your Fyers ID"},{key:"api_key",label:"App ID",hint:"From Fyers API dashboard"},{key:"api_secret",label:"Secret ID",hint:"From Fyers API dashboard",secret:true}], help:"Create App → paste Redirect URL → copy App ID & Secret" },
  zerodha: { name:"Zerodha", logo:"🔴", dashboard:"https://developers.kite.trade/", redirect:"https://mtutrade.in/api/auth/broker/zerodha/callback", fields:[{key:"user_id",label:"Zerodha ID",hint:"Your Zerodha ID"},{key:"api_key",label:"API Key",hint:"From Kite Connect"},{key:"api_secret",label:"API Secret",hint:"From Kite Connect",secret:true}], help:"Create App → whitelist IP 34.60.239.253 → paste Redirect URL → copy keys", note:"Whitelist IP: 34.60.239.253" },
}
...
export default function BrokerConnect({ T, user, onConnected }) {
  ...
  useEffect(()=>{
    ...
    if(!token) return
    fetch(`${API}/auth/broker/my-brokers`,{headers:{"Authorization":`Bearer ${token}`},credentials:"include"})
  ...
  async function connect(){
    ...
    for(const f of info.fields){ if(!fields[f.key]?.trim()){ setError(`Please enter ${f.label}`); return } }
      ...
      {step==="select" && (
        <div>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:10}}>
            ...
            <button onPointerDown={()=>{ if(selected){ setStep("setup"); setFields({}); setError("") } }} disabled={!selected}
        ...
        return (
          <div>
            ...
            <div style={{background:T.raised,borderRadius:8,padding:"12px",border:`1px solid ${T.line}`,marginBottom:8}}>
              ...
              <a href={info.dashboard} target="_blank" rel="noreferrer" style={{display:"block",textAlign:"center",padding:"10px",borderRadius:6,background:T.brand,color:"#fff",fontFamily:inter,fontWeight:700,fontSize:13,textDecoration:"none",marginBottom:6}}>Open {info.name} Developer Portal ↗</a>
            ...
            <div style={{background:T.raised,borderRadius:8,padding:"12px",border:`1px solid ${T.line}`,marginBottom:8}}>
              ...
              <div style={{display:"flex",gap:6,alignItems:"stretch"}}>
                <div style={{flex:1,background:T.surface,border:`1px solid ${T.line}`,borderRadius:6,padding:"8px 10px",fontFamily:mono,fontSize:9,color:T.ink,wordBreak:"break-all",lineHeight:1.5}}>{info.redirect}</div>
                <button onPointerDown={()=>copy(info.redirect)} style={{flexShrink:0,padding:"0 12px",borderRadius:6,border:"none",background:copied?"#2E7D32":T.brand,color:"#fff",fontFamily:inter,fontWeight:700,fontSize:12,cursor:"pointer",WebkitTapHighlightColor:"transparent",touchAction:"manipulation"}}>{copied?"✓":"Copy"}</button>
              </div>
            </div>
